...
#include <stdlib.h>
#include <string.h>
int hnsw_nSearch = 100000000;
size_t total_dist_calc = 0;
template <typename T>
...
namespace hnswlib {
  ...
  public:
    ...
    std::priority_queue< std::pair< dist_t, tableint  >, vector<pair<dist_t, tableint>>, CompareByFirst> searchBaseLayerST(tableint ep, void *datapoint, size_t ef) {
      ...
      while (!candidateSet.empty() && dist_calc < hnsw_nSearch) {
    ...
    std::priority_queue< std::pair< dist_t, labeltype >> searchKnn(void *query_data, int k) {
      ...
      for (int level = maxlevel_; level > 0; level--) {
        if (dist_calc >= hnsw_nSearch) break;
        bool changed = true;
        while (changed) {
          if (dist_calc >= hnsw_nSearch) break;
          changed = false;
      ...
      if(hnsw_nSearch<=dist_calc) {
        for(int i = 0; i < k; i++) {
          results.push(make_pair(0, -1));
        }
        dist_calc=0;
        return results;
      }
      ...
      if(results.size() < k) {
        int remain = k - results.size();
        for(int i = 0; i < remain; i++) {
          results.push(make_pair(0, -1));
        }
      }
      total_dist_calc += dist_calc;
      dist_calc=0;
      return results;
    };
    ...
    void       LoadIndex(const string &location, SpaceInterface<dist_t> *s)
    {
      ...
      /* hnsw graph extracting */
      vector<vector<int>> graphtmp = vector<vector<int>>(maxelements_, vector<int>());
      for (int i = 0; i < maxelements_; i++){
        int* data = (int*)(data_level0_memory_ + i * size_data_per_element_ + offsetLevel0_);
        int size = *data;
        int ext_i = getExternalLabel(i);
        for (int j = 1; j<=size; j++) {
          int tnum = *(data+j);
          graphtmp[ext_i].push_back(getExternalLabel(tnum));
        }
      }
     /* 
      std::ofstream output;
      output.open("hnswgraph.txt", std::ofstream::out);
      for (int i = 0; i < maxelements_; i++) {
        vector<int> neighbors = graphtmp[i];
