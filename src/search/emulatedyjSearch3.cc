...
public:
  ...
  static vector<int> sparsify(vector<int> &neighbor, int p) {
    vector<int> res;
    res.push_back(neighbor[0]);
    int end = 0;
    for(int i=1; i<neighbor.size(); i++) {
      bool test = true;
      int q = neighbor[i];
      float pq = inner_product(data[p], data[neighbor[i]], D);
      for(int j=0; j<res.size(); j++) {
        int r = res[j];
        //float pr = res[j].second;
        //insert if pq is not the longest edge == insert if  pq is not the furthest == insert if pq is not least similar
        float rq = inner_product(data[r], data[q], D);
        if(pq < rq) {
          test = false;
          break;
        }
    ...
    while(curNode != -1) {
      ...
      while(cont) {
        ...
        for (size_t i = 0; i < intersection_width; i++) {
          ...
          if(neighborNode == curNeighborNode)
            continue;
          if(sorted_graph[curNode].count(neighborNode) == 0)
            continue;
          vecvec.push_back(neighborNode);
          //if(vecvec.size() >= 32)
          //  break;
        }
        //assert(vecvec.size() !=0 );
        //if(vecvec.size() != 0)
        //  vecvec = sparsify(vecvec, curNeighborNode);
  ...
  void print_time() {
    cout << "[GraphSearchYJEmulated] " << (timers[1] + timers[2]) /queryCount << "(ms)\n";
